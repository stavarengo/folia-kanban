...
Raised neutral controls read `--interactive-normal` and `--interactive-hover` and retain the host input shadow. Bordered tiles and chips keep contrasting resting fills; flat links, icon buttons, suggestions and menu rows declare their shadow beside their own face. Chips and swatches keep their semantic geometry and selection rings. The host button and icon radiuses apply to their corresponding controls, while the accent focus outline follows the decision in `docs/decisions.md`.

`theme:check` calls `scripts/theme-buttons.mjs` to check JSX class branches, scoped face selectors and resting background/colour/shadow coverage. Only the ordinary button, filter chip and column-add classes deliberately inherit the host shadow. The check rejects bare button selectors that would reach rendered Markdown. It enforces the supported selector shape instead of calculating arbitrary specificity; it does not prove that arbitrary state refinements win against each other.

Hover and pressed outlines use a Folia-owned width and the control's label colour, because host fill pairs may be identical and shadows may be absent. Keyboard-selected suggestions also have a fixed-width inset marker. These signals are separate from selected-filter fills, choice rings and keyboard focus. `docs/decisions.md` records the measured collisions and the per-control choices.
