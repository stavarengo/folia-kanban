...
8. The plugin patches `WorkspaceLeaf.prototype.setViewState` so a board note opens as the board. Opening a board note as plain Markdown from a script therefore needs the plugin's own path: call `app.plugins.plugins['folia-kanban'].showMarkdownIn(leaf, '<vault path>')`.
