...
Neutral buttons use the theme's interactive fill; filter chips retain a distinct resting background and their selected tint. Open **Edit column** to compare Cancel and Save with buttons in Obsidian Settings; Save uses the accent fill. Menu rows stay flat, icon buttons remain compact, and keyboard focus keeps its accent outline. Check an active filter while hovering it: the accent tint must remain visible. Disabled actions must stay faded and must not gain a hover fill.

Button hover now adds a thin inset outline; pressing makes it thicker, including when a platform or theme gives resting and hovered buttons the same colour. Focus the search box and use the arrow keys: the selected suggestion has a left-edge marker even with transparent hover backgrounds. On the Parked column, the selected custom-colour swatch remains fully coloured although it is read-only. The dashed Add column tile keeps a transparent background so its outline stays visible.
