...
| **Next Up** | object column with a `color` — one of the eight theme colour names (`red`, `orange`, `yellow`, `green`, `cyan`, `blue`, `purple`, `pink`), so it follows whatever theme you are in |
...
| `Record the launch demo.md` | `blocked-by: ["[[Fix keyboard-drag focus bug]]"]` | the same edge stated from the other end — the bug card shows *Blocks 1*, with nothing written to it. This one is a nested subcard, and its own *Blocked* marker is what the **Blocked** filter below matches, whether or not its parent `Plan the v1.0 launch` also does |
...
Open any of them: the panel has one editable list per type (**Blocks**, **A result of** — type to get suggestions from the board's own cards) and one derived, read-only list for the other end (**Blocked by**, **Results in**). The plugin only ever writes the declaring end, so a link it created lives in one note and has no second copy to fall out of step with; an inverse you wrote by hand stays exactly as you wrote it. State the same link from both ends and the row says so rather than offering a remove button that would only clear half of it. A blocking marker fades once either end reaches **Done** — finished work neither waits nor holds anything up. Nothing is enforced: drag a *Blocked* card wherever you like, and press `/` and type `is:blocked` (or click the **Blocked** chip) to list what is waiting on something, `is:unblocked` for what is free to pick up. A filter matches every card on its own merits, however deep it is nested: `is:blocked` here answers with both `Draft the announcement post` and `Record the launch demo` — the latter lifted to the top level of its column, with a `↳ Plan the v1.0 launch` reference, because its parent is not itself blocked.
...
**Who is working on what** — three cards carry an `assignee`, which is a plain frontmatter key like any other:

| Card file | `assignee` | What you see |
| --- | --- | --- |
| `Plan the v1.0 launch.md` | `alex` | an **alex** chip on the tile, next to its priority and due chips |
...
- **Open a card** (click it) to see the **detail panel** — edit status, priority, due date, assignee, custom properties, subtasks, comments. Start typing in the empty **property** box at the end of the property list: the suggestions are Obsidian's own, and they come in three groups — what Folia Kanban understands, what the cards in `Cards/` already use (`energy`, `effort`), then the rest of the vault. The priority field is free text with the board's own values as suggestions: type `blocker`, and it joins the board note's `priorities` list and shows up as a suggestion from then on, even after you delete the card again. Try both presentations: Settings → **Card details** → *Show details in* → `side` vs `modal`.
- **Next actions on cards:** Settings → **Cards on the board** → *Next todos shown* → `3`. Cards now surface their next unchecked todos inline.
- **Collapse/expand subitems:** `Plan the v1.0 launch` has a nested subcard (`Record the launch demo`), so it carries a **Subitems** toggle under its title — click it to fold the subcard group away into a `4 subitems, 1 done` summary (that count is every `## Subtasks` line on the card, including the two placed elsewhere), click again to bring it back. Try Settings → **Cards on the board** → *Next todos shown* → `3` first, and it collapses the next-todos preview the same way. A column's `⋯` menu adds **Collapse all subitems** / **Expand all subitems** for everything shown in that column.
- **Unread comments:** open Settings → **Comments and history** → *Your name* and type `alex` — that is who the showcase vault pretends you are. Read-state lives in the plugin's own data, not in the notes, and tracking starts the moment the plugin first loads, so the comments shipped in this vault already count as read and nothing lights up by itself. Make something arrive: open `Cards/Decide the export format.md` as a note and add a line under `## Comments` stamped with the current date and time (or any later one), say `- _2030-01-01 09:00 @agent:_ any thoughts?` — and its tile's comment badge turns **blue** with a dot (*1 unread comment*). Do the same on `Cards/Plan the v1.0 launch.md`, which already holds two comments signed `@alex` (yours), and the badge turns **purple** with an arrow instead: *a reply to yours*. Open either card and the new comment is tinted and tagged, under a **New** rule; the tile goes quiet once you have visited it. Before that, click the **Unread** chip (or type `unread:comments`) to see only the cards with something new, `unread:replies` for only the answers to you.
- **Search:** press `/` (the board has to be the focused pane — the shortcut belongs to one board, not to the window) and try `priority:a`, `due:overdue`, `due:soon`, `area:work`, `tag:bug`, `tag:profiling` (written in a card's body rather than its `tags` property — a `#tag` in the text counts as one of the card's tags; the token still matches a whole tag, so `tag:work` does not find `#work/urgent`), `context:Engineering`, `assignee:alex`, `assignee:"ana maria"`, `assignee:none`, `is:blocked`, `is:unblocked`, `unread:comments`. Tokens **AND** together; quotes allow spaces (`area:"release plan"`); there's no negation beyond `is:unblocked`, `assignee:none` and `unread:none`. The **Mine** / **Overdue** / **Due soon** / **Blocked** / **Unread** chips are shortcuts for `assignee:me` / `due:overdue` / `due:soon` / `is:blocked` / `unread:comments` — **Mine** only appears once *Your name* holds a name, since without one there is no "me" to filter for.
...
- **Right-click** a card for the context menu (mark done, assign it to yourself, change priority, move, copy the card file's path in four forms, add subcard, delete). Right-click a surfaced next-todo to toggle it, remove it, or send it to a column of its own.
...
- **Swap the tab's view:** the tab header button flips between the board and the note's Markdown. Settings → **Boards and board notes** → *Open board notes as* picks which one a board note starts in, and adding `folia-view: markdown` to one board note's frontmatter overrides that for that note alone.
...
In light mode, cards, hover actions, menus, dragged cards and the detail panel use shorter, lighter shadows. Open Edit column to compare its backdrop with the dark-mode version. Check that the dialog remains distinct from the board without a heavy black veil. Compact icons should use consistent size and stroke weight across chips, menus and card metadata.

Also check the No priority chip in a card's context menu, drag a card to compare its overlay width with the source, and open a description containing a callout. Folia icons should stay consistent without changing the callout's own icon. Mini actions and colour swatches retain at least a 24px square target when the theme uses shorter inputs.

Neutral buttons use the theme's interactive fill; filter chips retain a distinct resting background and their selected tint. Open **Edit column** to compare Cancel and Save with buttons in Obsidian Settings: same height, radius, padding, fill and, now, the same 13px label. They are not meant to be indistinguishable — Folia's sit at weight 500 against the host's 400, and only Folia's answer the pointer with an inset outline — but no size step should separate them, and the dialog's own buttons and `<select>`s must agree with each other. Menu rows carry no fill at rest and start at the leading edge, exactly as Obsidian's own menu rows do — open a column menu and a card's right-click menu in **light** mode and compare them with any menu the app itself opens; a row should appear only under the pointer, in both schemes alike. Icon buttons remain compact, and keyboard focus keeps its accent outline. Check an active filter while hovering it: the accent tint must remain visible. Disabled actions must stay faded and must not gain a hover fill.

Button hover adds a thin inset outline; pressing draws it twice as thick and twice as deep, so the hovered and pressed rings sit in different places rather than differing only in weight. Hold the pointer down on Cancel and on Save and watch the ring move inward — it must stay on the button's own face, never on the surface around it, which is what keeps Save's white ring visible against its accent fill in light mode. This has to work even when a platform or theme gives resting and hovered buttons the same colour. Focus the search box: it should show **one** focus treatment, the field's own accent border and ring, with no second rectangle drawn inside it across the clear ✕. Then use the arrow keys: the selected suggestion has a left-edge marker even with transparent hover backgrounds. On the Parked column, the selected custom-colour swatch remains fully coloured although it is read-only. The dashed Add column tile keeps a transparent background, and keeps its dashed edge while hovered and pressed rather than gaining a solid one.

Open a column menu on **Todo**, which has no colour of its own and is painted from the board's automatic per-column colour: the **No colour** ✕ must carry the selection ring, since that is what the note actually stores — no palette swatch should be ringed. Compare with **Parked**, whose note stores a hex, and with any column given one of the eight. The same row is in **Edit column** and must agree with it.

In the card context menu, hover and press every priority choice, including No priority and a custom priority: each keeps its tint and gains the same thin/thick inset outline. No priority must remain a visible pill in light mode. Filter and column-choice chips keep their fills while hovered, and the Add column tile keeps its dashed edge. Open Add column, then move the pointer across the editing form: its background and border must stay unchanged.
