...
- **Unread comments:** open Settings → **Your name** and type `alex` — that is who the showcase vault pretends you are. `Plan the v1.0 launch` now reads as a conversation: two comments signed `@alex` (yours) and a later one from `@agent`, so its comment badge turns **purple** and says *a reply to yours*. `Decide the export format` has a single `@agent` comment and shows the plainer **blue** unread badge. Open either card and the new comments are tinted and tagged, under a **New** rule; the tile goes quiet once you have visited it. Read-state lives in the plugin's own data, not in the notes, so it is not shipped with this vault — before you set the name, every card with comments simply counts as unread, which is exactly what it is.
