...
Neutral buttons use the theme's interactive fill; filter chips retain a distinct resting background and their selected tint. Open **Edit column** to compare Cancel and Save with buttons in Obsidian Settings: same height, radius, padding and fill, and now the same 13px label, so the only way to tell them apart should be which window they are in. Menu rows carry no fill at rest and start at the leading edge, exactly as Obsidian's own menu rows do — open a column menu and a card's right-click menu in **light** mode and compare them with any menu the app itself opens; a row should appear only under the pointer, in both schemes alike. Icon buttons remain compact, and keyboard focus keeps its accent outline. Check an active filter while hovering it: the accent tint must remain visible. Disabled actions must stay faded and must not gain a hover fill.
