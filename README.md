...
A board wants width, so the sidebars are left out of all of this: a board note opened in the left or right dock stays Markdown there, and its button sends it to a real tab rather than squeezing columns into a dock. Opening the board from the command or the ribbon skips sidebar tabs for the same reason.

Three edge behaviours worth knowing. A note that gains or loses `folia-board` while it is open gains or loses the button in the Markdown editor, but the tab itself is left as it is rather than swapped out from under you — and a board that is already open keeps its **Edit as markdown** button either way, since that button is the way back to the text. A tab you left in the Markdown editor is still in the Markdown editor when Obsidian restarts: the setting says how a board note *opens*, and reopening the app is not you opening the note. And on the very first open of a session Obsidian may not have read the note's frontmatter yet; in that case the note opens as plain Markdown, and the button is right there to take you to the board.
