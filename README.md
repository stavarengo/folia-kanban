...
A subcard follows the same rule, across its two notes. When a child card reaches Done — dragged there, **Mark done**, or its status set from a detail panel — the plugin ticks the `- [ ] [[Child]]` line in every note that lists it, and a move out of Done unticks it again. That line is found by its link, never by its position in the list, so a parent you were editing at the same moment can at worst receive no write, never a tick on some other todo. The same rule read backwards: ticking a `[[Child]]` box in the detail panel sends a child that stands in a column of its own to Done, unticking one that sits in Done drops its `status` so it rejoins its card, and a child that claims nothing is left where it is — exactly what ticking an inline todo does to its `[status:: …]` field. Editing the child's `status` by hand in its note is the one path that writes nothing to the parent: the board never rewrites a note you did not touch through it. The parent's progress bar still counts that child as done while it sits in Done, and its checkbox catches up the next time the child moves through the board.
...
Any card can override all of that with a non-blank `title:` key in its own frontmatter, which wins in every mode. The detail panel's **Display title** field (also reachable from the card's right-click menu) sets and clears that key; its placeholder shows what the card is called without it. The `examples/title-modes/` folder holds three boards over the same three cards, one per mode, to compare them side by side.
