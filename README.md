...
Comments and history do not have to be bullets. A paragraph written by hand under `## Comments` — `**2026-08-21** — applied and checked`, say — shows in the panel as a comment with no timestamp or author; each paragraph is one comment, and it can be edited and deleted from the panel like any other. Editing writes it back as a single line, and deleting removes the whole paragraph. What stays invisible is a code block inside the section, and anything after the next heading.
