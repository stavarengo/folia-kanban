...
- [Columns that fill themselves](#columns-that-fill-themselves)
- [Who is working on this](#who-is-working-on-this)
...
priority: A
due: 2026-06-15
assignee: alex       # who is working on it
...
A column can carry its own rule instead of waiting for cards to be dropped on it: give it a `filter:` in the board note — `{ id: "research", title: "Research", filter: "area:research" }` — and it becomes a **lane**, drawing every card on the board that matches, wherever that card actually lives. One card can show up in several lanes at once, and in its own status column too; the rule decides, not the card.

Which is why a lane will not take a card you drag into it. A plain column owns the cards dropped on it, so a drop rewrites the card's `status` and there it stays. A lane owns nothing — it is a view of its rule — so a card that does not match the rule would land in a column that then refuses to draw it, and disappear off the board entirely. The drop is refused instead: the card snaps back, and a short message names the rule it does not match. Give the card what the rule asks for (set its `area`, its priority, whatever the rule reads) and it appears in the lane by itself, with nothing to drag. The same goes for every other way a card can be given a column: adding one straight into a lane, giving a checklist line a lane's column, deleting the column next door (its cards go to the nearest ordinary column, never into a lane), and an agent calling `move_card` or `create_card` through [Agent access](#agent-access-mcp).
...
A card can name several people — `assignee: [alex, ana]`, written by hand or by another tool — and that is read everywhere: a chip each on the tile, either name matching a filter, the panel listing them. The two controls then differ on purpose. **Assign to me** and **Unassign me** only ever add or remove your own name, leaving anyone else on the card alone — the one gesture the board makes about somebody else's assignment is none. The field beside them is a text box showing the names joined by commas, so typing over it writes exactly what you typed, as the one name it is: use the buttons to join or leave a shared card, and the note itself to write a list. (Which also means a name containing a comma — `Doe, Jane` — reads back from that field as two people. It is safe on the tile, in a filter and through the buttons; it is the field that cannot tell one from two.)

One thing to watch if something automated writes into your vault. A comment's `@name` prefix cannot hold spaces, so a name set as `Ana Maria` is signed `@Ana-Maria` — while `assignee` is ordinary frontmatter and keeps the spaces. Both are correct for where they are written, but they are two spellings, so a script that reuses the comment signature as an assignee writes `Ana-Maria`, which is a different person as far as the board is concerned. Assign with the name as the setting holds it.
...
- **Quick actions on every card** — mark done, open the note, or delete (with confirm) straight from the board, or right-click for the full context menu (change priority, move up/down, copy the card file's path, add subcard, and more). Opening a note is a normal Obsidian navigation, so the modifier keys mean what they mean everywhere else: Ctrl or Cmd click (or a middle click) opens it in a new tab, add Alt for a split and Alt+Shift for a new window. A plain click behaves as it always has — the board's own tab is never navigated away from, so the note lands in whichever other tab Obsidian would use.
...
- **Assignment** — say who is working on a card, in one click for yourself, from the panel or the card's right-click menu. It is an `assignee` key in the note's frontmatter, shown on the tile and filterable (`assignee:me`, `assignee:none`, a name); see [Who is working on this](#who-is-working-on-this).
- **Search & quick filters** — press `/` in the focused board (the shortcut belongs to one board, not to the window) to search by title, file name, tag, priority or assignee, or by token (`due:`, `priority:`, `assignee:`, `is:blocked`, `unread:comments`, …); one-click **Mine** / **Overdue** / **Due soon** / **Blocked** / **Unread** filters.
...
A column's `color` is one of Obsidian's eight theme colour names — `red`, `orange`, `yellow`, `green`, `cyan`, `blue`, `purple`, `pink` — and the column is painted with whatever your theme makes of that name, in light mode and in dark. Earlier versions wrote a hex instead. **A hex already in your board note keeps painting exactly the colour it always did** — nothing rewrites your note on its own — and the column menu shows it as an extra swatch beside the eight, so you can see what the column is carrying; picking any of the eight replaces it with a name.

What does change is the columns you never picked a colour for. The board colours those itself, and it now does it with theme colours, so they shift: mostly to the same hue drawn by your theme instead of by the plugin, except for two, where the old palette had colours the eight do not — a neutral grey, which becomes yellow, and a mint green, which becomes cyan.

One narrow case is worth knowing if you write your board notes by hand: the eight names are ordinary CSS colour words, so a column that already said `color: red` now takes the theme's red rather than pure `#ff0000`. That is deliberate — a board asking for red gets the red the rest of the app is using.
...
| Action | How |
| --- | --- |
| Search by title, file name, tag, priority or assignee | Press `/` |
