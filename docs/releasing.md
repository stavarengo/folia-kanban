...
4. **release** runs release-it on the approved commit: bump `manifest.json`/`versions.json`/`package.json`, write the `CHANGELOG.md` section, commit, tag and push — with `GITHUB_TOKEN`, whose pushes deliberately start no new workflow run. It then rebuilds (the manifest's version changed), attests build provenance for `main.js`, `styles.css` and `manifest.json`, creates the GitHub release with that changelog section as its notes, and uploads the three assets.
...
Start a release with `pnpm dev:helpers:release`, which dispatches the pipeline on `main` through the `gh` CLI and then watches the run in your terminal, approval pause included. It never bumps, tags or pushes anything locally. The same thing is two clicks away in the Actions UI: **Pipeline → Run workflow → main**.
...
**1. A required reviewer on the `release` environment.** The environment itself appears on its own the first time the pipeline runs, because the `approve` job names it — but an environment with no protection rule gates nothing, and every run would release straight through. Add the rule at **Settings → Environments → `release` → Deployment protection rules → Required reviewers**, add `stavarengo`, then **Save protection rules**. Leave **Prevent self-review** off: with it on, the person who started the run cannot approve it, and on a one-maintainer repository that means no release can ever be approved.
...
**2. A tag ruleset, so only the pipeline can create a version tag.** The pipeline is the only thing that should ever create a `*.*.*` tag; a ruleset is what turns that from a convention into a rule. Go to **Settings → Rules → Rulesets → New ruleset → New tag ruleset**, name it (`Release tags`), set **Enforcement status** to **Active**, under **Target tags** add a target with **Include by pattern** `*.*.*`, and tick the **Restrict creations** rule.
...
Then prove it before trusting it: the next release run has to get its tag pushed. If the `release` job fails on `git push` with a message about a ref protected by rules, the bypass did not take — widen or drop the ruleset. Nothing in this repository needs to change either way.
