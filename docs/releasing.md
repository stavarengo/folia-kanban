...
A "README contains unfilled placeholder text" warning did fire once, against `0.0.19`, over a stale "coming soon" blockquote left in the Install section after the plugin had already been accepted into `community-plugins.json`. Removing that blockquote in `0.0.20` closed it on the next rescan (confirmed 2026-08-28): the review reads Passed with no warnings.
