...
- **the OSV scanner workflows** (`osv-scanner-pr.yml`, `osv-scanner-scheduled.yml`): `security-events: write` and a read-only checkout, no secrets. Enough to file or dismiss a code-scanning alert — to hide a finding, not to release. Both are pinned to a commit SHA like everything else.
...
2. **Actions permissions** → **Allow *stavarengo*, and select non-*stavarengo*, actions and reusable workflows**. What is set on the repository today is the two sub-options ticked — **Allow actions created by GitHub** and **Allow actions by Marketplace verified creators** — plus this list:
   ...
   That is exactly what these workflows run today and nothing else could start, whoever opens the pull request. A reusable workflow is named in full, path included, so no wildcard is needed for the two OSV entries either. Its cost is upkeep: every pin here has to move with the workflow's, so a Dependabot bump's own run fails until this list is updated too — a nuisance with a clear message rather than a surprise, and the reason to keep the old SHA off the list once it is gone (see the republish note below, which is the one thing that makes an old pin matter). The `obsidianmd` line floats on purpose, which is also why the switch below, **Require actions to be pinned to a full-length commit SHA**, cannot be turned on: it would block the scan job.

   Whichever of the two is in force, keep this section and the setting saying the same thing. A document describing a stricter repository than the one configured is what gets discovered during an incident.
