...
2. **Actions permissions** → **Allow *stavarengo*, and select non-*stavarengo*, actions and reusable workflows**, with **both** sub-options left **unticked** — "Allow actions created by GitHub" admits every action in the `actions` and `github` organisations, and "Allow actions by Marketplace verified creators" admits every verified publisher's, which is most of the Marketplace. The box takes the same `owner/repo@ref` syntax as a workflow file, exact commit SHAs included, so name the pins rather than the namespaces:
   ...
   That is exactly what this workflow runs today, and nothing else can start, whoever opens the pull request. The cost is that this list has to be updated with the pins — a Dependabot bump will fail here until it is, which is a nuisance with a clear message rather than a surprise. The last line floats on purpose, which is also why the switch below, **Require actions to be pinned to a full-length commit SHA**, cannot be turned on: it would block the scan job.
