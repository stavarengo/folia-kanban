...
Who may release is now bounded by who may dispatch a workflow, which is anyone with write access to the repository — on a one-maintainer repository, the maintainer. The reviewer rule that used to bound it is gone along with the pause it caused.
