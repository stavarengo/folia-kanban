...
2. **Actions permissions** → **Allow _stavarengo_, and select non-_stavarengo_, actions and reusable workflows**, with **both** sub-options left **unticked** — "Allow actions created by GitHub" admits every action in the `actions` and `github` organisations, and "Allow actions by Marketplace verified creators" admits every verified publisher's, which is most of the Marketplace — and only this in the box: `actions/*`, `pnpm/action-setup@*`, `obsidianmd/obsidian-workflows@*`. That is everything this workflow uses. It narrows what a pull request can introduce to those three namespaces rather than to nothing: `actions/*` is a whole organisation, and the setting cannot be written per-SHA. The setting below it, **Require actions to be pinned to a full-length commit SHA**, would close that gap, and it is the one thing here you cannot turn on: the scan job's `obsidianmd/obsidian-workflows@v1` floats deliberately, and that policy would block it.
