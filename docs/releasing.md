...
The approval does not need a hand on a mouse. `pnpm dev:helpers:release --approve` dispatches the run and then answers the gate from the terminal, through the same API the button calls, as whoever `gh` is logged in as. That is how an agent asked to "cut a release" at the end of a coding cycle does it unattended: the gate is still there, still recorded as an approval by the maintainer, and still a pause for any run nobody approves. It only works for a login that is one of the environment's required reviewers; for anyone else GitHub refuses and the run keeps waiting in the UI. A run started by a push rather than by the script can be approved the same way by hand: `gh api --method POST repos/stavarengo/folia-kanban/actions/runs/<run id>/pending_deployments --input -` with `{"environment_ids":[<id>],"state":"approved","comment":"..."}`, where the id comes from `GET .../pending_deployments` on that run.

Do not approve two runs of the same release. Runs verify in parallel, but only one `release` job executes at a time, so a dispatched run's release queues behind the push run's; if the first one releases, the second stops at its own guard, because `main` has moved on since the commit it was started for.
