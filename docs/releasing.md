...
What you approve is then carried into the run, and enforced there. The dispatch passes the commit and the version as `expected_sha` and `expected_version`, and the release job refuses to release anything else: a dispatch names a branch, so GitHub resolves `main` to a commit when it creates the run, and the run reads the tags when it gets to them — a push or a new tag landing in those seconds would otherwise move what gets released without moving anything the run could notice. That is also why the script only *watches* the run it started rather than depending on having identified it correctly: GitHub's dispatch API returns nothing to correlate on, so with two dispatches in flight at once it can end up watching the other one, which is a cosmetic problem now rather than a safety one.
