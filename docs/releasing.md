...
2. **Actions permissions** → **Allow *stavarengo*, and select non-*stavarengo*, actions and reusable workflows**. What is set on the repository today is the two sub-options ticked — **Allow actions created by GitHub** and **Allow actions by Marketplace verified creators** — plus this list:
   ...
   google/osv-scanner-action@6e4298ebc4db23e847df9b2e2de2939d6f066c67
   google/osv-scanner-action/*
   obsidianmd/obsidian-workflows@v1
   ```

   That is exactly what these workflows run today and nothing else could start, whoever opens the pull request. Its cost is upkeep: every pin here has to move with the workflow's, so a Dependabot bump's own run fails until this list is updated too — a nuisance with a clear message rather than a surprise. (`google/osv-scanner-action/*` stays as a path pattern because those two entries are reusable workflows inside that repository, referenced by file path.) The `obsidianmd` line floats on purpose, which is also why the switch below, **Require actions to be pinned to a full-length commit SHA**, cannot be turned on: it would block the scan job.
