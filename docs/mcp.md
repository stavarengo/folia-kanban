...
Folia Kanban can host a [Model Context Protocol](https://modelcontextprotocol.io) server inside the plugin, so an AI agent drives your boards the way you drive them: it creates cards, moves them, sets fields, comments and ticks subtasks — and every one of those goes through the board's own rules. A card an agent touches gets the same history lines, the same signed comments — signed with the agent's own name, not yours — and the same fractional ordering it would have got from you dragging it. That is the whole point: an agent editing the card files directly does none of that, because it has no idea any of it is owed.
...
| `get_board` | reads | A board's columns and the cards in each, in the order the board shows them, each with its priority, due date and `assignee`. A card nested under another one is in no column of its own, so it is listed under its parent's `children`. A checklist line standing in a column of its own is listed as a card of kind `todo`, with its `line`: the reading `move_card` holds a move of it to. |
| `get_card` | reads | One card in full: column, frontmatter, description, subtasks, comments, history, relationships. Each subtask carries its `index`, its `text` and its `occurrence`, which of the lines reading exactly that text it is. Asked about a checklist line's card, it reports that line's `line`, the same one `get_board` does. |
...
| `add_comment` | writes | Appends a comment to `## Comments`, timestamped and signed with the `author` the call gives — required, and the agent's own name rather than the user's. One line: a comment is stored as a single list item. |
| `add_subtask` | writes | Appends an unchecked line to `## Subtasks`, and reports the `index`, `text` and `occurrence` of the line it added — what `set_subtask_done` asks for. One line, for the same reason. |
| `set_subtask_done` | writes | Ticks or unticks one subtask, named by BOTH the `index` and the `text` `get_card` reported for it, and optionally its `occurrence`. A line claiming a column of its own is kept in step with its checkbox. |

`set_subtask_done` takes the line's words as well as its position, and refuses the write when the note no longer reads that way. An index is only a position: anything added or removed above a line — by hand, by sync, by another tool, or by an earlier call in the same loop — moves every line below it, and a tick decided against an old reading would land on somebody else's todo. So read the card, pass back what it said, and treat a refusal as the signal to read it again rather than as a failure to work around: a write it refuses does not land. Where the line's own `[status:: …]` claim ends up is not part of what you pass, and not decided from your reading either: ticking a line that claims a column moves that claim to the done column, unticking one that claims the done column drops it, and a line claiming nothing is left claiming nothing — worked out from the claim the note carries at the moment the write lands. So a claim somebody changed between your read and the write does not send the line somewhere decided before that change, and is not a refusal either. Two limits are worth knowing. Words cannot tell apart two checklist lines that read exactly the same, which is what `occurrence` is for: pass it too, and a different line added or removed above a pair of identical lines is refused rather than landing on the other one. What even that cannot see is an edit that slides an identical line into your line's place — a twin inserted directly above it, one inserted or removed further up while your line has a twin right next to it, or yours deleted with its twin right below — since the position, the words and the count all move together (see [Decisions](decisions.md#identical-checklist-lines-are-told-apart-by-count-not-by-identity)). And ticking a line that claims a column of its own is two writes rather than one, so a note edited in the moment between them can leave the box written and only the claim refused; the message says which half that was.

`move_card` holds a checklist line to the same kind of reading, and does not move one without it. Such a line is named by its position in its note (`Note.md#todo:2`, or its words, which resolve to that same position), and a position names whatever line sits there by the time the move lands: a line removed above it during your think time would hand the move to the line below. So a checklist-line card needs its `line`, exactly as `get_board` or `get_card` reported it — its `index`, `text`, `occurrence`, `done` and, when it claims one, the `status` it claims — and the move is refused, with nothing written, when the note no longer reads that way at that position, or when the line claims a different column, or has its box ticked or unticked differently, than the reading your move replaces. A move your reading says is already done writes nothing, and is refused the same way when the note no longer reads like that reading. Read the card again and move what is there now. A move without a `line` is refused with a message saying so, and so is a `line` handed along with a card that is a note of its own, or one describing a line that links a card of its own (move that card instead). A `Note.md#todo:N` path is taken at its word even when the board draws no card for that line — its `line` is what the move is held to — so the same call also gives a line still living with its card a column of its own.
...
`add_comment` requires `author`, the name the comment is signed with, and the server never falls back to the user's own. A comment's signature is what tells the board whose it is: signed with the reader's **Your name**, a comment is read as one of their own, so it is never marked unread and a genuine answer landing after it never reads as a reply — an agent's note would arrive silently, in the user's voice. Give your own name (`codex`, `release-bot`) and it lands as somebody else's, which is what it is. The name is written inside `- _<time> @name:_`, so it has to be one word: whitespace, `:`, `@`, `*`, a backtick and square brackets are refused rather than quietly rewritten, so what the board attributes the comment to is exactly what the call asked for. A leading `@` is the one thing forgiven — `@codex` and `codex` both sign as `codex`.

A column carrying a `filter:` rule is not a column a card can be put into. It is an auto-populated **lane**: the board fills it with every card matching that rule, wherever the card's own `status` says it lives, and it never draws its own status bucket. So `create_card` and `move_card` refuse a target lane the card does not match, quote the rule, and write nothing — a card whose `status` named a lane that would not draw it used to end up in no column at all. Give the card what the rule asks for with `update_card` first, or send it to a column with no rule of its own. `get_board` reports a lane as the board draws it, with a `lane` note repeating that the listing is the rule resolved and not a bucket the cards belong to; one card can appear in several lanes and in its own status column at once.

The exception is a rule naming `unread:` or `assignee:me`. Both read state that lives with the person at the board — which comments they have seen, and the name in their **Your name** setting — and the server can see neither. It does not refuse over a rule it cannot read; the write goes through with a `warning` saying which part it could not judge.
