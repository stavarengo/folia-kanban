...
`set_subtask_done` takes the line's words as well as its position, and refuses the write when the note no longer reads that way. An index is only a position: anything added or removed above a line — by hand, by sync, by another tool, or by an earlier call in the same loop — moves every line below it, and a tick decided against an old reading would land on somebody else's todo. So read the card, pass back what it said, and treat the refusal as the signal to read it again rather than as a failure to work around: the refused write does not land. Two things the words cannot do. They cannot tell apart two checklist lines that read exactly the same — between those, the index still decides. And ticking a line that claims a column of its own is two writes, so a note edited in the moment between them can leave the box written and its claim refused; the message says so when that happens.
