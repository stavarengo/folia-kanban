...
One asymmetry to know: a card can name several people (`assignee: [alex, ana]`, written by hand or by the board's own **Assign to me** button), and `get_board` and `get_card` both report that list — but `properties` values are scalars, so a tool cannot write one back. Assigning through this server therefore means assigning one person, and doing it to a card that already names two replaces both. Read the value before you write it, and leave a list alone unless replacing it is what was asked for.
