...
**Anything newer than 1.7.2 is a lint error unless it is gated.** `eslint.config.mjs` enables `obsidianmd/no-unsupported-api`, which reads `minAppVersion` straight out of `manifest.json`. Every facility this report marks `since 1.8.7` or later therefore needs the pattern `src/main.ts:1054` already uses — `requireApiVersion("1.13.0")` with a fallback — or a raised floor. This is why several obvious-looking fixes (`ConfirmationModal`, `App.secretStorage`, `saveLocalStorage`, `Setting.setErrorMessage`, `ButtonComponent.setDestructive`) are trade-offs rather than clear wins.
